...
app.get('/tasksapi',(req,res)=>{
	const tasks = todoService.listTasks().map(task=>({id:task.id, name:task.name, done:task.done}))// el metodo listTask retorna un array de tareas
	res.json({
		status: 'OK',
		message: 'Tasks list succesfully',
		data: tasks// aqui tasks es una tarea con su id, name y done
	})
})
